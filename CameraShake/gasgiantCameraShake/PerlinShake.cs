...
namespace CameraShake
{
    ...
    {
        /// <summary>(1f, 1f, 0.5f) </summary>
        public static Vector3 defaultEuler => new Vector3(1f, 1f, 0.5f);
        /// <summary>Euler scale defaults to (1, 1, 0.5) if left null.</summary>
        public static Displacement GetRotationDisplacement(float strength, Vector3? eulerScale = null)
        {
            if (eulerScale == null) eulerScale = defaultEuler;
            return new Displacement(Vector3.zero, eulerScale.Value * strength);
        }
        ...
        /// <summary>Creates an instance of PerlinShake.</summary>
        ...
        public void UpdateSourcePosition(Vector3 sourcePosition) => this.sourcePosition = sourcePosition;
        public void SetStrength(Displacement strength) => pars.strength = strength;
        public Envelope.EnvelopeState State => envelope.State;
        public void ForceSustain() => envelope.ForceSustain();
        public void ForceDecay() => envelope.ForceDecay();
        public void ForceDecay(float decayDuration)
        {
            decayDuration = Mathf.Max(decayDuration, 0.00001f);
            envelope.SetDecay(1f / decayDuration);
            envelope.ForceDecay();
        }
        public void NoDecay() => envelope.SetDecay(0f);
