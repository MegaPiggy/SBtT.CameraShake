...
namespace CameraShake
{
    ...
    public class Envelope : IAmplitudeController
    {
        ...
        bool forceSustainQueued;
        public void ForceSustain()
        {
            if (state == EnvelopeState.Increase)
            {
                forceSustainQueued = true;
                return;
            }
            state = EnvelopeState.Sustain;
            sustainEndTime = Time.time;
            amplitude = initialTargetAmplitude;
            targetAmplitude = 0f;
        }
        public void SetDecay(float decay) => pars.decay = decay;
        public void ForceDecay()
        {
            forceSustainQueued = false;
            state = EnvelopeState.Decrease;
            sustainEndTime = 0f;
            targetAmplitude = 0f;
        }
        ...
        public Envelope(EnvelopeParams pars, float initialTargetAmplitude, EnvelopeControlMode controlMode)
        {
            ...
            this.initialTargetAmplitude = initialTargetAmplitude;
        }
        ...
        public bool IsFinished
        {
            get
            {
                ...
                return (finishWhenAmplitudeZero || controlMode == EnvelopeControlMode.Auto)
                    && amplitude <= 0f && targetAmplitude <= 0f;
            }
        }
        ...
        public void Update(float deltaTime)
        {
            ...
            {
                if (state == EnvelopeState.Decrease && !forceSustainQueued)
                {
